...
function allowPrivatePluginFetches(): boolean {
  return process.env.PAPERCLIP_PLUGIN_FETCH_ALLOW_PRIVATE === "true";
}
...
async function validateAndResolveFetchUrl(urlString: string): Promise<ValidatedFetchTarget> {
  ...
  try {
    ...
    const safeResults = allowPrivatePluginFetches()
      ? results
...
export function buildHostServices(
  ...
  options: { pluginWorkerManager?: PluginWorkerManager; containerService?: ContainerService; manifest?: import("@paperclipai/shared").PaperclipPluginManifestV1 } = {},
